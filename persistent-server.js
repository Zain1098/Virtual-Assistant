...
app.get('/', (req, res) => {
  res.json({ message: 'Shifra AI Smart Assistant with Data Storage!' });
});
  ...
  socket.on('voice-command', async (data) => {
    ...
    try {
      ...
      if (command.includes('hello') || command.includes('hi') || command.includes('hey')) {
        response = { response: `Hello ${storage.userName}! How can I assist you today?`, action: 'speak' };
      }
      ...
      // Name setting
        ...
        response = { response: `Nice to meet you, ${name}!`, action: 'speak' };
      }
      ...
      // Task management
        ...
        const result = storage.addTask(task);
        response = { response: result, action: 'speak' };
      }
        ...
        const result = storage.deleteTask(taskText);
        response = { response: result, action: 'speak' };
      }
      ...
      // Reminders
        ...
        const result = storage.addReminder(reminder, 'later');
        response = { response: result, action: 'speak' };
      }
      ...
      // System info
        ...
        response = { 
          response: 'I can help you with: opening websites, checking weather, converting currency, calculations, managing tasks, setting reminders, telling jokes, playing music, and answering questions!', 
          action: 'speak' 
        };
      }
    ...
    // Save conversation
    storage.saveConversation(data.command, response.response);
...
server.listen(PORT, () => {
  ...
  console.log(`💾 Data storage enabled - all conversations and tasks saved`);
  console.log(`🧠 Smart features: Weather, Currency, Tasks, Reminders, Stats`);
});